...
- **多模态输入**：用户发的图片会先压缩到 256KB 以下，再以 data URL 形式直接喂给主模型；`forward_to_vision: true`（默认，主模型非多模态时）且 vision 已启用时，图片会存到临时文件并转发给 vision 工具查看（调用结束后自动删除）;vision 未启用时图片仍原样内嵌发给主模型。文档/视频/音频只把元数据（文件名、大小、视频时长、消息 ID）告诉主模型，不下载本体
- **工具调用**:
...
- **按钮审批**：每次调用工具前都会弹出 Telegram 内联按钮，用户点「同意」才执行；超时未点按拒绝处理。所有工具（包括只读的 read）都必须经过审批
...
| `forward_to_vision` | 是否把用户发来的图片转发给 vision 工具（即主模型本身非多模态、不能直接看图），默认 `true`；为 `true` 且 vision 已启用时，图片存到临时文件（`$TMPDIR/agent-ying/`），由主 agent 调 vision 工具查看（调用结束后自动删除）；为 `false` 或 vision 未启用时图片原样内嵌发给主模型 |
