...
impl Tool for Write {
    ...
    fn description(&self) -> String {
        "写入文件:不存在则创建,存在则覆盖,自动创建父目录。路径必须是绝对路径(不接受相对路径)。只在创建新文件或完整重写时使用 write;小改动用 edit 工具".into()
    }
...
impl Tool for Edit {
    ...
    fn description(&self) -> String {
        "对单个文件做精确文本替换:每条 edits[].oldText 必须原样唯一命中、且彼此不重叠,全部编辑对着同一份原始文件匹配(非增量)。同一处或相邻行有多处改动时合并成一条编辑,不要发重叠/嵌套编辑;oldText 尽量小而唯一,不要为衔接远处的改动带大段不变内容。路径必须是绝对路径(不接受相对路径);同一文件改多处时,用一次 edit 调用的 edits[] 带多条,而不是多次调用".into()
    }
